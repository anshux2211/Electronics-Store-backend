...
public class CartControllers {
    ...
    @Operation(description = "This API adds a specified product to a user's cart. The client must provide the user ID as a path variable, along with the product ID and quantity as request parameters. Upon successful addition, the updated cart details are returned.")
    ...
    @Operation(description = "This endpoint allows the removal of a specific item from a user's cart. It requires the user ID and the cart item ID as path variables. After successfully removing the item, a confirmation message is returned.\n" +
            "\n")
    ...
    @Operation(description = "This API clears all items from a user's cart. It takes the user ID as a path variable and returns a success message once the cart has been emptied.")
    ...
    @GetMapping("/{userId}")
    @Operation(description = "This endpoint retrieves the current contents of a user's cart. The user ID is passed as a path variable, and the API returns the full cart details including all items and their quantities.")
