...
import static com.lcwd.electronic.store.config.AppConstants.ADMIN;
import static com.lcwd.electronic.store.config.AppConstants.NORMAL;
...
public class OrderControllers {
    ...
    @PostMapping("/create")
    @Operation(description = "This endpoint allows an admin user to create a new order. It accepts a CreateOrderRequest object in the request body, which contains all the necessary details to place an order. Upon successful creation, the API returns the newly created OrderDto along with HTTP status 201 Created.")
    ...
    @GetMapping
    @Operation(description = "This API fetches a paginated list of all orders in the system, accessible to users with NORMAL or ADMIN roles. It supports pagination and sorting through optional query parameters: pageNumber, pageSize, sortBy, and sortDir, and returns a PageableResponse of OrderDto.")
