...
public class CategoryControllers {
    ...
    //save
    @PostMapping("/save")
    @Operation(description = "This API creates a new category in the system. It accepts a CategoryDto object in the request body, validates it, and then delegates the creation task to the service layer. Upon successful creation, the newly created category object is returned with HTTP status 201 Created.\n" +
            "\n")
    ...
    @Operation(description = "This endpoint updates the details of an existing category. It takes the category ID as a path variable and the updated CategoryDto as the request body. On successful update, it returns the updated category data with HTTP status 200 OK.")
    ...
    @Operation(description = "This API deletes a category by its ID. The category ID is passed as a path variable. If the deletion is successful, it responds with a confirmation message and HTTP status 200 OK.")
    ...
    //get all
    @GetMapping
    @Operation(description = "This endpoint retrieves a paginated list of all categories. It supports optional query parameters like pageNumber, pageSize, sortBy, and sortDir to control pagination and sorting. It returns a paginated response with category details.")
    ...
    @Operation(description = "This API fetches a single category by its ID. The category ID is provided as a path variable. If found, it returns the category’s details with HTTP status 200 OK.")
    ...
    @Operation(description = "This endpoint uploads an image for a specific category. It accepts the image file as a multipart form data and the category ID as a path variable. If the category exists, the image is uploaded and linked to the category, and a success response with image details is returned.")
    ...
    @Operation(description = "This API serves the image associated with a given category ID. It retrieves the image from the file system and streams it back in the HTTP response with a Content-Type of image/jpeg.")
